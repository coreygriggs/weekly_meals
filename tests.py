...
class MealsTest(unittest.TestCase):
    ...
    def test_pass_new_ingredient_with_ingredient_name(self):
        ...
        post_request = requests.post(self.base_url + '/ingredients', data=data)
    ...
    def test_update_ingredient_new_name(self):
        data = {'ingredient_name': self.rand_string}
        base_ingredient = requests.post(self.base_url + '/ingredients', data=data)
        updated_ingredient = {'ingredient_name': self.rand_string}
        updated = requests.patch(self.base_url + '/ingredients/%s' % base_ingredient.json().keys()[0], data=updated_ingredient)
        self.assertEqual(updated_ingredient['ingredient_name'], updated.json().values()[0])

    def test_index_returns_success(self):
        get_index = requests.get(self.base_url)
